...
SDL_bool mon_addsym( struct symboltable *stab, unsigned short addr, unsigned short flags, char *name, struct msym **symptr )
{
  ...
  if( ( !stab->syms ) || ( stab->symspace < (stab->numsyms+1) ) )
  {
    ...
    newbuf = (msym *)malloc( sizeof( struct msym ) * (stab->symspace+64) );
  ...
  {
    stab->syms[stab->numsyms].name = (char *)malloc( 128 );
...
static SDL_bool mon_console_keydown( SDL_Event *ev, struct machine *oric, SDL_bool *needrender )
{
  ...
  switch( ev->key.keysym.sym )
  {
    ...
    case SDLK_KP_ENTER:
      ...
      tzsetcol( tz[TZ_MONITOR], 2, 3 );
      done = done ? SDL_TRUE : (SDL_bool)mon_cmd( ibuf, oric, needrender );
      mon_start_input();
      mon_show_curs();
      *needrender = SDL_TRUE;
      break;
  ...
  switch( ev->type )
  {
    ...
    case SDL_KEYDOWN:
      ...
      if( !donkey )
      {
        ...
        {
          case CSHOW_CONSOLE:
            done = done ? SDL_TRUE : (SDL_bool)mon_console_keydown( ev, oric, needrender );
            break;

          case CSHOW_MWATCH:
            done = done ? SDL_TRUE : (SDL_bool)mon_mwatch_keydown( ev, oric, needrender );
            break;
        }
      }
      break;
