  ...
  for( y=0; y<224; y++)
  {
    ...
    for( b=0; b<40; b++ )
    {
      ...
      if( ( c & 0x60 ) == 0 )
      {
        ...
        ula_render_block( oric, (SDL_bool)((c & 0x80)!=0), 0, y );
        if( y < 200 )
        {
          if( oric->vid_mode & 0x04 ) // HIRES?
          {
        ...
        if( hires )
        {
          ula_render_block( oric, (SDL_bool)((c & 0x80)!=0), c & bitmask, y );
        } else {
          int ch_ix, ch_dat;
          ...
          ula_render_block( oric, (SDL_bool)((c & 0x80)!=0), ch_dat, y );
        }
      }
    }
  }
...
SDL_bool ula_doraster( struct machine *oric )
{
  ...
  for( b=0; b<40; b++ )
  {
    ...
    {
      ula_decode_attr( oric, c, y );
      oric->vid_block_func( oric, (SDL_bool)((c & 0x80)!=0), 0, y );
      if( y < 200 )
      {
        if( oric->vid_mode & 0x04 ) // HIRES?
        {
      ...
      if( hires )
      {
        oric->vid_block_func( oric, (SDL_bool)((c & 0x80)!=0), c & bitmask, y );
      } else {
        int ch_ix, ch_dat;
        ...
        oric->vid_block_func( oric, (SDL_bool)((c & 0x80)!=0), ch_dat, y );
      }
    }
  }
