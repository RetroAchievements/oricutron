...
SDL_bool init( struct machine *oric, int argc, char *argv[] )
{
  ...
  sto = (start_opts *)malloc( sizeof( struct start_opts ) );
      ...
      do {
        switch( event.type )
        {
          ...
          default:
            ...
            {
              case EM_MENU:
                done = done ? done : menu_event( &event, &oric, &needrender );
                break;

              case EM_RUNNING:
                done = done ? done : emu_event( &event, &oric, &needrender );
                break;

              case EM_DEBUG:
                done = done ? done : mon_event( &event, &oric, &needrender );
                break;
            }
        }
