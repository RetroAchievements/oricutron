...
//  printf("wavsize = %d\n", datalen);
  ...
  {
    smp = getsmp(&data[i], (SDL_bool)(bps==1));
    if (smp < sminl) sminl = smp;
    if (smp > smaxl) smaxl = smp;

    if (stereo)
    {
      smp = getsmp(&data[i+bps], (SDL_bool)(bps==1));
  ...
  if (dcoffsav)
  {
    lastsmps = (int *)malloc(dcoffsav * sizeof(int));
...
//  fflush(stdout);
  ...
  ortbuf = (unsigned char *)malloc(ortlen);
...
SDL_bool tape_load_tap( struct machine *oric, char *fname )
{
  ...
  oric->tapebuf = (unsigned char *)malloc( oric->tapelen+1 );
  if( !oric->tapebuf )
  {
    fclose( f );
    oric->tapelen = 0;
...
void tape_patches( struct machine *oric )
{
  ...
  if( oric->romon )
  {
    if( oric->pch_fd_available )
    {
      ...
      {
        SDL_bool justtap = (SDL_bool)(oric->tsavf != oric->tapecap);
        tape_stop_savepatch( oric );
        if( justtap )
        {
