...
static SDL_bool writeblock(SDL_bool stillok, FILE *f)
{
  ...
  stillok = (SDL_bool)(fwrite(buf, offs, 1, f) == 1);
  offs = 0;
  return stillok;
}
...
static SDL_bool datablock(SDL_bool stillok, unsigned char *data, Uint32 len, FILE *f)
{
  ...
  stillok  = (SDL_bool)(fwrite("DATA", 4, 1, f) == 1);
  stillok = (SDL_bool)(stillok && (fwrite(&lenbe, 4, 1, f) == 1));
  stillok = (SDL_bool)(stillok && (fwrite(data, len, 1, f) == 1));
...
SDL_bool save_snapshot(struct machine *oric, char *filename)
{
  ...
  buf = (unsigned char *)malloc(MAX_BLOCK);
  if (!buf)
  {
...
static struct blockheader *load_block(struct machine *oric, char *id, FILE *f, SDL_bool required, int expectedsize, SDL_bool datarequired)
{
  ...
  bkh[i].buf = (unsigned char *)malloc(bkh[i].size);
  if (!bkh[i].buf)
  {
...
SDL_bool load_snapshot(struct machine *oric, char *filename)
{
  struct m6502 *cpu = &oric->cpu;
  int i;
  SDL_bool do_wd17xx = SDL_FALSE, back2mon = (SDL_bool)(oric->emu_mode == EM_DEBUG);
  ...
  oric->romdis         = (SDL_bool)getu8 (blk);
  oric->romon          = (SDL_bool)getu8 (blk);
  oric->vsynchack      = (SDL_bool)getu8 (blk);
  blk->offs++; // Skip drivetype (already got it)
  oric->tapeturbo      = (SDL_bool)getu8 (blk);
  ...
  for (i=0; i<8; i++)
    oric->ay.keystates[i] = (SDL_bool)getu8(blk);
  for (i=0; i<3; i++)
  ...
  oric->via.t1run    = (SDL_bool)getu8 (blk);
  oric->via.t2run    = (SDL_bool)getu8 (blk);
  oric->via.ca2pulse = (SDL_bool)getu8 (blk);
  oric->via.cb2pulse = (SDL_bool)getu8 (blk);
  oric->via.srtrigger= (SDL_bool)getu8 (blk);
  ...
  oric->tapemotor   = (SDL_bool)getu8 (blk);
  oric->tapeturbo_forceoff= (SDL_bool)getu8(blk);
  oric->rawtape     = (SDL_bool)getu8 (blk);
  ...
  {
    oric->tapebuf = (unsigned char *)malloc(blk->datablock->size);
    if (!oric->tapebuf)
    {
  ...
  if ((blk = load_block(oric, "PCH\x00", f, SDL_FALSE, 76, SDL_FALSE)))
  {
    ...
    oric->pch_fd_available               = (SDL_bool)getu8(blk);
    ...
    oric->pch_tt_readbyte_setcarry       = (SDL_bool)getu8 (blk);
    oric->pch_tt_available               = (SDL_bool)getu8 (blk);
    oric->pch_tt_save_available          = (SDL_bool)getu8 (blk);
  ...
  switch (oric->drivetype)
  {
    ...
    case DRV_MICRODISC:
      ...
      oric->md.diskrom = (SDL_bool)getu8(blk);
    ...
    case DRV_PRAVETZ:
    {
      ...
      for (i=0; i<2; i++)
      {
        ...
        oric->pravetz.drv[i].dirty       = (SDL_bool)getu8(blk);
        oric->pravetz.drv[i].prot        = (SDL_bool)getu8(blk);
      }
      ...
      while ((blk = load_block(oric, "PVD\x00", f, SDL_FALSE, -1, SDL_FALSE)))
      {
        ...
        oric->wddisk.disk[i]->rawimage = (Uint8 *)malloc(blk->datablock->size);
        if (!oric->wddisk.disk[i])
        {
  ...
  if (do_wd17xx)
  {
    ...
    oric->wddisk.last_step_in = (SDL_bool)getu8 (blk);
    getdata(blk, disks, 4);
    oric->wddisk.currentop    = getu32(blk);
  ...
  if (oric->type == MACH_TELESTRAT)
  {
    ...
    oric->tele_via.t1run    = (SDL_bool)getu8 (blk);
    oric->tele_via.t2run    = (SDL_bool)getu8 (blk);
    oric->tele_via.ca2pulse = (SDL_bool)getu8 (blk);
    oric->tele_via.cb2pulse = (SDL_bool)getu8 (blk);
    oric->tele_via.srtrigger= (SDL_bool)getu8 (blk);
