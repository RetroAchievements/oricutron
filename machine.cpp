...
void setemumode( struct machine *oric, struct osdmenuitem *mitem, int mode )
{
  ...
  switch( mode )
  {
    case EM_RUNNING:
      ...
      oric->ay.soundon = (SDL_bool)(soundavailable && soundon && (!warpspeed));
      if( oric->ay.soundon )
      {
        ay_flushlog( &oric->ay );
        SDL_PauseAudio( 0 );
...
void setromon( struct machine *oric )
{
  ...
  if( oric->drivetype == DRV_JASMIN )
  {
    ...
    {
      oric->romon = (SDL_bool)(!oric->romdis);
    } else {
  ...
  {
    oric->romon = (SDL_bool)(!oric->pravetz.olay);
  }
  else if (oric->type == MACH_TELESTRAT)
  {
    oric->romon = (SDL_bool)(oric->tele_banktype == TELEBANK_ROM);
  }
}
  ...
  else if( 0x320 <= addr && addr <= 0x3ff )
  {
    switch( addr )
    {
      case 0x380:
        ...
        oric->romdis = (SDL_bool)!oric->pravetz.romdis;
        oric->romon = (SDL_bool)!oric->romdis;
        break;
      case 0x381:
        oric->pravetz.olay = 1;
...
SDL_bool isram( struct machine *oric, unsigned short addr )
{
  ...
  {
    return (SDL_bool)(oric->tele_banktype == TELEBANK_RAM);
  }
...
static SDL_bool load_rom( struct machine *oric, char *fname, int size, unsigned char *where, struct symboltable *stab, int symflags )
{
  ...
  tmpname = (char *)malloc( strlen( fname ) + 10 );
    ...
    case SDL_KEYUP:
      switch( ev->key.keysym.sym )
      {
        ...
        case SDLK_F6:
          ...
          {
            ay_flushlog( &oric->ay );
            oric->ay.soundon = (SDL_bool)(!warpspeed);
        ...
        case SDLK_F10:
           ...
           warpspeed = SDL_FALSE;
           ay_lockaudio( &oric->ay );
           vidcap = avi_open( &vidcapname[13], oricpalette, (SDL_bool)(soundavailable&&soundon), oric->vid_freq );
           ay_unlockaudio( &oric->ay );
           if( vidcap )
           {
             vidcapcount++;
...
typedef unsigned char(*Read)(struct m6502 *, Uint16);
typedef void(*Write)(struct m6502 *, Uint16, Uint8);

static void setup_for_microdisc( struct machine *oric, void *readptr, void *writeptr )
{
  oric->cpu.read = (Read)readptr;
  oric->cpu.write = (Write)writeptr;
  oric->romdis = SDL_TRUE;
  microdisc_init( &oric->md, &oric->wddisk, oric );
...
SDL_bool init_machine( struct machine *oric, int type, SDL_bool nukebreakpoints )
{
  ...
  switch( type )
  {
    case MACH_ORIC1_16K:
      ...
      oric->mem = (unsigned char *)malloc( oric->memsize );
      if( !oric->mem )
      {
